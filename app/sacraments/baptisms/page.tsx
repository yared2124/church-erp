...
import { Baby, UserRound, User, CalendarCheck, Plus } from "lucide-react";
...
export default function BaptismsPage() {
  ...
  React.useEffect(() => {
    setLoading(true);
    apiFetch<{ data: SacramentStatsResponse }>("/api/sacraments/stats?type=Baptism")
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        title="Sacraments"
        actions={<Button icon={<Plus size={16} />}>Add Baptism</Button>}
      />
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Total Baptisms" value={loading ? "…" : String(stats?.total ?? 0)} icon={Baby} iconBg="bg-primary-light" iconColor="text-primary" />
      ...
      <div className="mb-5">
        <SacramentsWorkspace type="Baptism" priestOptions={stats?.priests ?? []} />
      </div>
